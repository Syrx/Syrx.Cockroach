namespace Syrx.Cockroach.Tests.Integration
{
    public class DatabaseBuilder
    {
        ...
        {
            Throw<ArgumentNullException>(!string.IsNullOrWhiteSpace(name), nameof(name));
            // For CockroachDB, we create all tables at once rather than parameterized creation
            // Only execute the table creation once (when name is "poco")
            if (name == "poco")
            {
                _commander.Execute<bool>();
            }
        ...
        {
            Throw<ArgumentNullException>(!string.IsNullOrWhiteSpace(name), nameof(name));
            // For CockroachDB, use direct TRUNCATE without parameters
            _commander.Execute<bool>();
            return this;
        }
